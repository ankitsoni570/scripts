...
url = 'https://dev.azure.com/<org>/<project>/_settings/agentqueues?__rt=fps&__ver=2'
try:
...
def get_our_agents_name(pools):
    result = []
    for pool in pools:
        if pool['owner']['displayName'] == 'Ankit Soni' or pool['owner']['displayName'] == 'Azure Pipelines':
...
def send_mail():
    ...
    to_emails = [
        ('ankit.soni@example.com', 'Ankit Soni'),
    ...
    if mail_flag == "true":
        data_ = '<html><head><style>table {font-family: arial, sans-serif; border-collapse: collapse; width: 100%;} td {' \
                ...
                'detail.</p>' \
                '<br/> <table>' + mail_body + '</table><br/>Thanks and Regards</p><p>Infra ' \
                                            'Team</p></body></html>'
        message = Mail(
            from_email='infra@example.com',
            to_emails=to_emails,
            is_multiple=True,
            subject=subject,
            html_content=data_
